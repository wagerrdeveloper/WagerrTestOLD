...
bool CWallet::SelectCoins(const CAmount& nTargetValue, set<pair<const CWalletTx*, unsigned int> >& setCoinsRet, CAmount& nValueRet, const CCoinControl* coinControl, AvailableCoinsType coin_type, bool useIX) const
{
    ...
    //if we're doing only denominated, we need to round up to the nearest .1 WGR
    if (coin_type == ONLY_DENOMINATED) {
        // Make outputs by looping through denominations, from large to small
        BOOST_FOREACH (CAmount v, obfuScationDenominations) {
            BOOST_FOREACH (const COutput& out, vCoins) {
                if (out.tx->vout[out.i].nValue == v                                               //make sure it's the denom we're looking for
                    && nValueRet + out.tx->vout[out.i].nValue < nTargetValue + (0.1 * COIN) + 100 //round the amount up to .1 WGR over
                    ) {
                    CTxIn vin = CTxIn(out.tx->GetHash(), out.i);
                    int rounds = GetInputObfuscationRounds(vin);
                    // make sure it's actually anonymized
...
bool CWallet::SelectCoinsByDenominations(int nDenom, CAmount nValueMin, CAmount nValueMax, std::vector<CTxIn>& vCoinsRet, std::vector<COutput>& vCoinsRet2, CAmount& nValueRet, int nObfuscationRoundsMin, int nObfuscationRoundsMax)
{
    ...
    if (!(nDenom & (1 << 0))) fFound25000 = true;
    ...
    BOOST_FOREACH (const COutput& out, vCoins) {
        ...
        if (nValueRet + out.tx->vout[out.i].nValue <= nValueMax) {
            ...
            //
            // bit 0 - 25000 WGR+1 ( bit on if present )
            // bit 1 - 25000 WGR+1 ( bit on if present )
            // bit 2 - 1000 WGR+1
            // bit 3 - 100 WGR+1
            // bit 4 - 10 WGR+1
            // bit 5 - 1 WGR+1
            ...
            if (fFound25000 && fFound10000 && fFound1000 && fFound100 && fFound10 && fFound1 && fFoundDot1) { //if fulfilled
                ...
                if ((nDenom & (1 << 0)) && out.tx->vout[out.i].nValue == ((25000 * COIN) + 10000000)) {
                    fAccepted = true;
    ...
    */
    ...
    while (nStep < nStepsMax) {
        BOOST_FOREACH (CAmount v, obfuScationDenominations) {
            ...
            bool fAccepted = false;
            if ((obfuScationPool.sessionDenom & (1 << 0)) && v == ((25000 * COIN) + 10000000)) {
                fAccepted = true;
                         ...
                         CZerocoinSpendReceipt& receipt, libzerocoin::SpendType spendType, CBlockIndex* pindexCheckpoint)
{
    ...
    if (!GenerateAccumulatorWitness(pubCoinSelected, accumulator, witness, nSecurityLevel, nMintsAdded, strFailReason, isV1Coin, pindexCheckpoint)) {
        receipt.SetStatus(_("Try to spend with a higher security level to include more coins"), ZWGR_FAILED_ACCUMULATOR_INITIALIZATION);
    ...
    try {
        ...
        if (!spend.Verify(accumulator)) {
            receipt.SetStatus(_("The new spend coin transaction did not verify"), ZWGR_INVALID_WITNESS);
            //return false;
        ...
        //Add the coin spend into a Wagerr transaction
        ...
        try {
            ...
            receipt.SetStatus(_("Failed to deserialize"), ZWGR_BAD_SERIALIZATION);
            return false;
        }
...
bool getV1bnAccumulatorValue(CoinSpend newSpend, CBigNum& bnAccumulatorValue)
{
    ...
    // 2. Get pubcoin from the private coin
    libzerocoin::CoinDenomination denomination = newSpend.getDenomination();
    libzerocoin::PublicCoin pubCoinSelected(paramsCoin, newSpend.GetValue(), denomination);
    //LogPrintf("%s : selected mint %s\n pubcoinhash=%s\n", __func__, zerocoinSelected.ToString(), GetPubCoinHash(zerocoinSelected.GetValue()).GetHex());
    if (!newSpend.validate()) {
        LogPrintf("The selected mint coin is an invalid coin");
        return false;
    }
