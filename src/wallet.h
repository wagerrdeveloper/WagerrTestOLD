...
enum AvailableCoinsType {
    ...
    ONLY_DENOMINATED = 2,
    ONLY_NOT25000IFMN = 3,
    ONLY_NONDENOMINATED_NOT25000IFMN = 4, // ONLY_NONDENOMINATED and not 25000 WGR at the same time
    ONLY_25000 = 5,                        // find masternode outputs including locked ones (use with caution)
    STAKABLE_COINS = 6                          // UTXO's that are valid for staking
};

// Possible states for zWGR send
enum ZerocoinSpendStatus {
    ZWGR_SPEND_OKAY = 0,                            // No error
    ZWGR_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZWGR_WALLET_LOCKED = 2,                         // Wallet was locked
    ZWGR_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZWGR_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZWGR_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZWGR_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZWGR_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZWGR_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZWGR_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    ZWGR_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZWGR_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
...
//    bool getV1bnAccumulatorValue(CoinSpend newSpend, CBigNum& bnAccumulatorValue);
    ...
    bool CreateZWGROutPut(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint);
    ...
    */
    ...
    bool CreateTransaction(CScript scriptPubKey, const CAmount& nValue, CWalletTx& wtxNew, CReserveKey& reservekey, CAmount& nFeeRet, std::string& strFailReason, const CCoinControl* coinControl = NULL, AvailableCoinsType coin_type = ALL_COINS, bool useIX = false, CAmount nFeePay = 0, const std::string& opReturn = "");
